...
if TYPE_CHECKING:
    from .models import Error as ScryfallError
...
class UnparsedAPIError(Error):
    ...
    def __init__(self, status: int, details: bytes):
        super().__init__(f"Scryfall API returned unparsable error: {details}")
        self.status = status
...
class APIError(Error):
    ...
    def __init__(self, status: int, error: "ScryfallError"):
